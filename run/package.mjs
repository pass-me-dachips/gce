...
const sharedPath = join("distrib", "shared");
...
const compileCommand = `nim c -d:release --app:console --outDir:distrib --opt:speed ${gcePath}`;
execSync(compileCommand);

const installPath = join("run", "install.mjs");
copyFileSync(installPath, join("distrib", "install.mjs"));
