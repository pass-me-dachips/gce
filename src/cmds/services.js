...
function s2() {
  ...
  const cb = (pong, elem) => {
    if (pong !== null && pong === elem) {
      ...
      const serviceDetails = JSON.parse(
        readFileSync(servicePath, SYSTEM.encoding)
      );
      const {
        servicePort: port,
        serviceId: id,
      ...
      const mem = serviceDetails?.memUsage?.heapUsed
        ? hmr(serviceDetails?.memUsage?.heapUsed)
        : "no records";
      const tmem = serviceDetails?.memUsage?.rss
        ? hmr(serviceDetails?.memUsage?.rss)
        : "no records";
  ...
  if (services.length > 0) {
    console.log(
      "\x1b[93ms-mem > total memory currently used by the service.\x1b[0m"
    );
    console.log(
      "\x1b[96mr-mem > total memory currently used by the runtime including the s-mem, v8 engine, external components etc.\x1b[0m"
    );
  }
  return void 0;
}
  ...
  const cb = (pong) => {
    if (pong !== null && pong === serviceId && existsSync(servicePath)) {
      ...
      const bw = serviceDetails?.fs?.total_bytes_written
        ? hmr(serviceDetails?.fs?.total_bytes_written)
        : "no records";
      const bd = serviceDetails?.fs?.total_bytes_dropped
        ? hmr(serviceDetails?.fs?.total_bytes_dropped)
        : "no records";
      const dw = serviceDetails?.fs?.total_dirs_written
        ? serviceDetails?.fs?.total_dirs_written
        : "no records";
      const dd = serviceDetails?.fs?.total_dirs_dropped
        ? serviceDetails?.fs?.total_dirs_dropped
        : "no records";
      console.log(`service (${id})`);
      console.log(
        SYSTEM.tabA +
          `port = \x1b[92m${port}\x1b[0m, id = \x1b[92m${id}\x1b[0m, temp = \x1b[92m${isTemp}\x1b[0m`
      );
      console.log(
        SYSTEM.tabA +
          `path = \x1b[92m${path}\x1b[0m, type = \x1b[92m${type}\x1b[0m`
      );
      console.log(
        SYSTEM.tabA +
          `gcce = \x1b[92m${gcce}\x1b[0m, v = \x1b[92m${version}\x1b[0m, cat = \x1b[92m${createdAt}\x1b[0m`
      );
      console.log(
        SYSTEM.tabA +
          `total bytes-written \x1b[92m(${bw})\x1b[0m, total bytes-dropped \x1b[92m(${bd})\x1b[0m`
      );
      console.log(
        SYSTEM.tabA +
          `total dirs-written \x1b[92m(${dw})\x1b[0m, total dirs-dropped \x1b[92m(${dd})\x1b[0m`
      );
      console.log(
        SYSTEM.tabA +
          `uptime = \x1b[92m${
            uptime ?? "no records"
          }\x1b[0m, idle-since = \x1b[92m${idleSince ?? "no records"}\x1b[0m`
      );
      console.log(
        SYSTEM.tabA +
          `pid = \x1b[92m${Pid}\x1b[0m, s-mem = \x1b[92m${mem}\x1b[0m, r-mem = \x1b[92m${tmem}\x1b[0m \n`
      );
