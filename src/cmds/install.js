...
export default function Install(args, cwd) {
  if (args.length === 2) {
    ...
    if (existsSync(gconfigPath)) {
      const gconfigContent = JSON.parse(
        readFileSync(gconfigPath, SYSTEM.encoding)
      );
      ...
      if (
        ...
        if (
          ...
          gcceRegistry.forEach((elem) => {
            if (
              elem.name === gconfigContent.name &&
              elem.version === gconfigContent.version
            ) {
              shouldWriteFile = false;
              let message = `${elem.name} with version ${elem.version} already installed.`;
              throw { message };
            } else {
              shouldWriteFile = true;
              contentToWrite.push(elem);
            }
          });
          if (shouldWriteFile) {
        ...
        const message =
          "gcce requirements not met.\nare you sure the gcce's gconfig.json has the following fields? \x1b[92mstart, author, name, repo, version, entry, notFound, forbiden\x1b[0m";
        throw { message };
      }
