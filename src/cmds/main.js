...
export default async function Main(args, cwd) {
  try {
    ...
    let port_raw = serviceOptions
      .find((elem) => elem.startsWith("--port"))
      ?.split(":");
    if (port_raw === undefined || port_raw.length !== 2) {
      servicePort = 0;
    ...
    if (typeof gcces === "object" && gcces.length > 0) {
        ...
        if (gcces.length === 1) {
          ...
          gcces.forEach((elem) => {
            console.log(
              `${
                SYSTEM.tabA
              }name ${`\x1b[36m${elem.name}\x1b[0m`}, version ${`\x1b[33m${elem.version}\x1b[0m`}`
            );
          });
          ...
          const prompt =
            "- specify a gcce and its version in which you want to use eg = <gccename>,<version> ";
          let gcceNV = await rlAsync(prompt); //NV as in name, version
          const gcceSelected = (gcceNV) => {
            let gcce = gcces.find(
              (elem) =>
                elem?.name === gcceNV?.split(",")[0] &&
                elem?.version === gcceNV?.split(",")[1]
            );
            return gcce;
          };
          while (!gcceSelected(gcceNV)) {
            gcceNV = await rlAsync(prompt);
          }
          rl.close();
      ...
      *** let gconfigContent be its content instead of the global config content.
      ...
      if (defaultKey in gconfigContent === false) await specifyGcce();
        ...
        defaultKey in gconfigContent &&
        findGcce(gconfigContent.default) !== null
      ) {
      ...
      console.log(
        "\x1b[93m\n~ Published at %s, Copyright (c) 2024 by David.A",
        SYSTEM.releaseDate
      ); //@log
      console.log("~ Official repository at: %s", SYSTEM.homeRepo); //@log
      console.log(
        "~ Hint: run gce --help to see all available commands and options\n\x1b[0m"
      ); //@log
