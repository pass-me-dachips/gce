...
async function add(relativePath, cwd) {
  if (relativePath) {
    ...
    const Headerfile = JSON.parse(
      readFileSync(pathToHeaderfile, SYSTEM.encoding)
    );
    let {
      author,
      name,
    ...
    if (
      ...
      if (requirements.length > 0) {
        ...
        await questionAsync(
          "\x1b[95mare you sure you met the specified requirements?\x1b[0m hit enter to continue "
        );
        rl.close();
      }
      const osplatform = platform();
      if (
        supportedPlatforms.includes(osplatform) &&
        ...
        while (!existsSync(PATHS.pkgs)) {
          mkdirSync(PATHS.pkgs, { recursive: true });
        }
        if (existsSync(pkgs_registry_path)) {
          const previousContents = JSON.parse(
            readFileSync(pkgs_registry_path, SYSTEM.encoding)
          );
          if (name in previousContents) {
            console.log(`Alert: ${name} already added to the registry.`);
            console.log(
              `Gce would upgrade or update the previous entry with this.`
            );
        ...
        console.log(
          `\x1b[5;94m${name} does not support platform: ${osplatform}\x1b[0m`
        );
        process.exit(1);
      }
    } else
      console.log(
        `package does not meet gce's requirements.\nvisit \x1b[95m${SYSTEM.buildAPackage}\x1b[0m to learn more about Headerfiles`
      );
    process.exit(1);
  } else
    console.log(
      "cannot add a package without its relative path to the source code.\nHint: make sure you run \x1b[92m`gce pkg add <relative_path_to_package>`\x1b[0m next time!"
    );
  process.exit(1);
}
...
function remove(packageName) {
  if (existsSync(pkgs_registry_path)) {
    ...
    if (packageName) {
      ...
      console.log(
        "\x1b[92myou are about to remove all existing packages\x1b[0m"
      );
      const cb = (answer) => {
        rl.close();
        if (answer === "ACK") {
  ...
  if (existsSync(pkgs_registry_path)) {
    ...
    if (!packageName) {
      ...
      packagesName.forEach((elem) => {
        console.log(
          `\x1b[92m${elem}\x1b[0m  ${packages[elem].version}  \x1b[95m${packages[elem].cmd1}\x1b[0m`
        );
      });
    } else {
      ...
      if (packageInfo) {
        ...
        console.log(
          "want to update this package? \x1b[92mrun gce pkg add path/to/package\x1b[0m"
        );
        console.log(
          "want to remove this package? \x1b[92mrun gce pkg remove %s \x1b[0m\n",
          packageName
        );
      } else
        throw {
          message: `could not retreive info about package ${packageName}. \x1b[93mare you sure it exists?\x1b[0m`,
        };
    }
  } else throw { message: "no packages found" };
}
...
export default function Pkg(args, cwd) {
  ...
  if (args[0]) {
    const option = args[0];
    const sub_command_arg = args[1];
    switch (option) {
      case "add":
        add(sub_command_arg, cwd);
        break;
      case "man":
        man(sub_command_arg);
        break;
      case "remove":
        remove(sub_command_arg);
        break;
      case "show":
        show(sub_command_arg);
        break;
      default:
        throw { message: `invalid sub-command ${option}` };
    }
  } else {
