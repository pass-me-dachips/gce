...
export default function Gcce() {
  if (existsSync(PATHS.gcceConfig)) {
    const registry = JSON.parse(
      readFileSync(PATHS.gcceConfig, SYSTEM.encoding)
    );
    let defaultGcce = "cannot determine";
    if (existsSync(PATHS.globalConfig)) {
      const globalConfig = JSON.parse(
        readFileSync(PATHS.globalConfig, SYSTEM.encoding)
      );
    ...
    for (let i = 0; i < registry.length; i++) {
      let gcce = registry[i];
      console.log(
        `${SYSTEM.tabA}gcce ${gcce?.name}, v ${gcce?.version}, repo \x1b[95m${gcce?.repo}\x1b[0m`
      );
    }
